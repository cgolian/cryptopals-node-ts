...
export function decryptCtrCiphertextsEncryptedWithSameNonce(base64Ciphertexts: ReadonlyArray<string>): Array<Buffer> {
    ...
    transposed.forEach((block, byteNr) => {
        ...
        const { keyByte } = breakSingleByteXOR(block.toString('hex'));
        keystream[byteNr] = keyByte;
    });
    // try decrypting ciphertexts with key
    // return in original order
    const decrypted: Array<Buffer> = [];
    trimmed.forEach((ciphertext: Buffer) => {
        ...
        decrypted.push(BitArray.toBuffer(plaintext));
    });
    return decrypted;
}